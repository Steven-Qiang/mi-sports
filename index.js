 ...
 * -----
 * @last-modified: 2022-07-11 01:06:49
 * -----
 */
...
async function main(username, password, step) {
  ...
  const data_json = // cspell-checker:disable-next-line
...
(async () => {
  for (const opt of config) {
    if (opt.step_fixed.enable && opt.step_range.enable) {
      console.log(opt.username + ',step_fixed,step_range不能同时开启。');
      continue;
    }
    if (!opt.step_fixed.enable && !opt.step_range.enable) {
      console.log(opt.username + ',step_fixed,step_range必须开启一个。');
      continue;
    }
    const step = opt.step_range.enable ? getRandomInt(opt.step_range.start, opt.step_range.end) : opt.step_fixed.number;
    await main(opt.username, opt.password, step);
  }
})();
